...
**Key risks.**
- *Bouncer over-drops.* The salary gate must drop only when `salary_max > 0 AND < the configured floor`; unlisted (`0`/NULL) must pass to the Icebox. Snapshot-test the five filters against fixtures.
...
**Demo.** Run the sourcing daemon for one cycle against the live Tier-A APIs; show the console drop-log (reason per rejected job) and `uv run aeroapply rank` proving core-title roles rank above adjacent-title ones.
