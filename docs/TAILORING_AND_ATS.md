...
> **Implementation status (#34).** Landed: the full embedding path — `src/aeroapply/embeddings.py` (provider-agnostic `Embedder`, default OpenAI `text-embedding-3-small`, deterministic `HashEmbedder` for offline/tests, startup dimension guard), `chunk_resume`, `repo.index_resume_chunks` / `repo.retrieve_resume_chunks` (pgvector `<=>`), the `aeroapply index` command, the `retrieve` node that **grounds the Generator** on the chosen variant's top-k chunks, AND **embedding-ranked variant selection**: `make_db_variant_selector` scores each variant by the mean chunk similarity above, and `select_resume` picks the best (recording `selection_method='embedding'`), falling back to the deterministic `role_focus`/title-substring pick when no variant is indexed or clears the floor. One simplification vs. the SQL above: scoring iterates variants in Python over `retrieve_resume_chunks` rather than the single windowed query, and `min_similarity` defaults to 0.0 ("best non-empty wins") — a real floor and the `agent_confidence` lowering on fallback are calibration follow-ups (#58, #72), not yet wired.
...
After tailoring passes (or caps), the `cover_letter` node (`src/aeroapply/nodes/cover_letter.py`) drafts a letter via a separate Generator call (node `cover_letter` → drafting class), grounded on the just-finalized tailored résumé and the job. It inherits the same honesty constraint **as a prompt instruction**: the letter is told to assert only what the tailored résumé substantiates. Output is written to `application.cover_letter` (the `tailored` audit event flags whether one was produced). The cover letter is *not* re-scored by the ATS-Critic and — unlike the résumé, which has the Generator⇄Critic loop — has **no automated honesty/overstatement verification yet**; its truthfulness currently rests on the prompt plus the operator's review before submit (the M3 Inbox). A cover-letter critic/overstatement check is a follow-up; until then, treat the draft as unverified.
